  ...
  module Patches::GroupPatch
    ...
    module InstanceMethods
      def add_and_remove_members_to_group(action_user, new_user_ids, remove_user_ids)
        ...
        call = Groups::UpdateService.new(user: action_user, model: self).call(
          add_user_ids: (new_user_ids - current_user_ids).uniq,
          remove_user_ids: (remove_user_ids & current_user_ids).uniq
        )

        call.on_success do
          Rails.logger.debug "[ThPlugin groups] Added users #{new_user_ids} to #{name}"
          Rails.logger.debug "[ThPlugin groups] Removed users #{remove_user_ids} from #{name}"
        end
